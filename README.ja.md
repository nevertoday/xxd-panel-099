<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 099 プロジェクトバナー" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 099｜書体マスコット
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 099 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
- 人物、動物、物、場所を小さくても識別できるブランドマスコットにしたい場合
- 3〜5個の特徴、丸い幾何形、太い輪郭、誇張比率で強い Shape Language を作りたい場合
- 超大文字と元写真由来の2〜4色でキャンペーン視覚を組みたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 099 で処理し、最適な構図とサイズを先に提案してください。
...
- 太く清潔で少しデジタル手描きの弾力を持つ黒輪郭と、大きな純色色面でキャラクターを構築します。
- キャラクターを絶対的な主役とし、超大文字は遮られ、交差し、裁ち落とされて版面構造になります。
- 元写真から識別性の高い2〜4色を抽出してブランド化し、すべての色をそのまま複製しません。
- 吹き出しは物語上の価値がある場合だけ使い、補助文字は小さく節度を保ちます。
- 写実陰影、複雑な体積、アニメ、3D、プラスチック風グラデーション、汎用ステッカー、テンプレートSNS版面を避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-099-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 099 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-099 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-099 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-099 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-099 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 099 を適用し、共通設定は一度だけ解決
/xxd-panel-099 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-099`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-099-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-099-prompt.zh-CN.md)
