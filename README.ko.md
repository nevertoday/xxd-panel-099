...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 099이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
- 사람·동물·사물·장소를 작은 크기에서도 알아볼 수 있는 브랜드 마스코트로 만들고 싶을 때
- 3–5개 특징, 둥근 기하, 굵은 윤곽, 과장 비율로 강한 Shape Language가 필요할 때
- 초대형 타이포와 원본에서 뽑은 2–4색으로 캠페인 비주얼을 구성할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 099로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
- 굵고 깨끗하며 약간의 디지털 손그림 탄성을 지닌 검정 윤곽과 큰 순색 면으로 캐릭터를 만듭니다.
- 캐릭터를 절대적 주인공으로 두고 초대형 글자는 가려지고 교차하며 잘려 화면 구조가 됩니다.
- 원본에서 식별성 높은 2–4색을 추출해 브랜드화하며 모든 원색을 그대로 복사하지 않습니다.
- 말풍선은 서사적 가치가 있을 때만 쓰고 보조 글자는 작고 절제합니다.
- 사실 명암, 복잡한 부피, 애니메이션풍, 3D, 플라스틱 그라데이션, 범용 스티커, 템플릿 소셜 레이아웃을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-099-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 099의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-099 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-099 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-099 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-099 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-099 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-099 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 099 적용, 공통 설정은 한 번만 확정
/xxd-panel-099 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-099`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-099-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-099-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

99개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 099까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–099) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
