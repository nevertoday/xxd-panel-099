<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 099 项目横幅" width="1200"></p>
...
```text
辨认主体与叙事核心 → 提炼 3–5 个稳定特征 → 用圆润几何与 Shape Language 角色化 → 以角色为绝对主视觉组织超大文字 → 从原图提炼 2–4 色品牌系统 → 让少量气泡或图形信息服务于构图
```

## 成品中最容易识别的风格特点

- 粗重、干净、略带数字手绘弹性的黑色轮廓与大片纯色色块构成角色骨架。
- 角色始终是绝对主视觉；超大文字可被遮挡、穿插或裁切，承担版面结构而非孤立标题。
- 配色从原图提炼 2–4 个最有辨识度的颜色并品牌化，不照搬全部原色。
- 气泡只在有趣或有叙事价值时出现，不机械添加；辅助文字小而克制。
- 拒绝写实明暗、复杂体积、动漫、3D、渐变塑料、普通贴纸素材和模板化社交版式。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-099-prompt.en.md)
...
| **[xxd-panel-099](https://github.com/nevertoday/xxd-panel-099)** | Flat Vector Mascot · 粗黑轮廓 · 圆润几何 · 3–5 个身份特征 · 2–4 色品牌系统 · 超大字 · 可选气泡 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–099） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
